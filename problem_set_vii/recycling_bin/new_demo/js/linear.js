    ...
    var search = setInterval(function() {
      ...
      if (++i >= list.length || found) {
        if (i === list.length && !found) {
          ...
          if (!found) {
            document.getElementById('search-results').innerHTML = 'SEARCH RESULTS: ' + target + ' was not found.';
          } else {
            document.getElementById('search-results').innerHTML = 'SEARCH RESULTS: ' + target + ' was found at index ' + (i - 1) + '.';
          }
        }
      } else {
